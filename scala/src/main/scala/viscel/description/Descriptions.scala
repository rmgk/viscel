...
object Description {
	def fromOr(or: List[Description] Or Every[ErrorMessage]): List[Description] = or.fold(Predef.identity, Failed(_) :: Nil)
	...
	final case class Core(kind: String, id: String, name: String, metadata: Map[String, String]) extends Description
	final case class Failed(reason: Every[ErrorMessage]) extends Description
}
