...
import loci.communicator.ws.javalin._
...
trait JavalinWS extends WS {
  val javalinContext: WsContext
}

private object LociJavalinWSHandler {
  locally(LociJavalinWSHandler)
  ...
  def connectionAttribute(
                           ctx: WsContext,
                           connection: Connection[JavalinWS],
  ...
  def handleConnection(
                        ...
                        connectionSetup: ConnectionSetup[JavalinWS],
                        connectionEstablished: Try[Connection[JavalinWS]] => Unit): Unit = {
    wsHandler.onConnect(new WsConnectHandler {
      override def handleConnect(ctx: WsConnectContext): Unit = {

        // heartbeat

        val timeout = properties.heartbeatTimeout.toMillis.toLong
        ...
        val connection = new Connection[JavalinWS] {
          val protocol = new JavalinWS {
            val javalinContext = ctx
            val path = wsPath
            val host = None
            val port = None
