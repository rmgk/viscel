...
object Mangadex extends Metarrator[MangadexNarrator]("Mangadex") {
  ...
  case class ChapterInfo(lang_code: String, chapter: String, volume: String, title: String) {
    ...
    def sorting: (Int, Int, String, String) =
      (volume.toIntOption.getOrElse(Int.MaxValue), chapternumber, chapter, title)
  }
  ...
  val archiveWrapper: Wrapper = {
    ContextW.map { context =>
      ...
      json.chapter.toList.filter(_._2.lang_code == "gb").sortBy(_._2.sorting).flatMap {
        case (id, chap) => contents(id, chap)
      }
    }
  }
  ...
  val pageWrapper: Wrapper = {
    ContextW.map { context =>
      ...
      chapter.page_array.zipWithIndex.map {
        case (fname, i) =>
